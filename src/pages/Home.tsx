import { ActionIcon, Button, Checkbox, CheckboxCard, CheckboxGroup, Container, Divider, Group, List, LoadingOverlay, Menu, PasswordInput, Popover, Select, Stepper, Table, Text, Textarea, TextInput, Tooltip } from "@mantine/core";
import { useEffect, useState } from "react";
import Header from "../components/Header";
import { IconArrowRight, IconCapProjecting, IconDotsVertical, IconDownload, IconLogout2, IconUser } from "@tabler/icons-react";
import { Navigate, NavLink, useNavigate } from "react-router-dom";
import { useForm } from "@mantine/form";
import supabase, { autoCancelAppointment } from "../supabase";
import { AppointmentProps, AppointmentTimeProps, DisabledDateProps, ReasonProps, SectionProps, UserProps } from "../types";
import { notifications } from "@mantine/notifications";
import { displayConfirmation, displayError, displaySuccess, formatDate, generateRandomString, LoadingClass, toProper } from "../helpers/methods";
...
function Home() {
  ...
  const [appointmentState, { open: openAppointmentState, close: closeAppointmentState }] = useDisclosure(false)
  ...
  const [account, setAccount] = useState<UserProps | undefined | null>(undefined)
  ...
  function reset() {
    setSteps({ step1: undefined, step2: undefined, step3: undefined, step4: undefined });
  ...
  async function submitActiveAccountForm(rs: ActiveAccountProps) {
    ...
    if (rs.password) {
      const { error: passError } = await supabase.auth.admin.updateUserById(rs.auth_id, {
  ...
  const fetch = async () => {
    ...
    const all_appointments = (await supabase.from("appointments").select("*")).data;
    const appointmentTime_ = (await supabase.from("appointment_time").select("*").order("id")).data;
    ...
    if (!rs) {
      ...
      const app = (await supabase
        .from("appointments")
        .select("*")
        .eq("student_id", rs.id).order('updated_at', {
          ascending: false
        })
      ).data;
      setAppointments(app || [])
    ...
    modals.open({
      ...
      children: (
        <div>
          ...
          <Text mb={8} ff="montserrat-bold">{appointment.note || '-'}</Text>
          ...
          <Text mb={8} ff="montserrat-bold">{`${formatDate(new Date(appointment.appointment_date))} | ${appointment.appointment_time}`}</Text>
        </div>
      )
    })
  }

  async function forgotPasswordEventHandler({ email }: ForgotPasswordFormProps) {
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === "INSERT") {
            setAppointments((prev) => [payload.new as AppointmentProps, ...prev]);
          } else if (payload.eventType === "UPDATE") {
            setAppointments((prev) =>
              prev.map((item) =>
                item.id === payload.new.id ? payload.new as AppointmentProps : item
              )
            );
  ...
  if (account === undefined) {
    return (
      <div className="flex items-center justify-center w-full h-screen font-montserrat-bold">Loading...
      </div>
    )
  }
  ...
  const pendingAppointments = allAppointments.filter((ap) => ap.student_id === account?.id && ap.status.toLowerCase() === 'pending');
  ...
  return (
    <Container>
      ...
      <CustomModal title="Account" opened={accountState} onClose={closeAccountState}>
        <form className="space-y-3" onSubmit={activeAccountForm.onSubmit(submitActiveAccountForm)}>
          <Group grow justify="start">
            <TextInput {...activeAccountForm.getInputProps('firstname')} required label="First Name" placeholder="Enter First Name" />
            <TextInput {...activeAccountForm.getInputProps('lastname')} required label="Last Name" placeholder="Enter Last Name" />
          </Group>
          <Group grow justify="end">
            ...
            <DateInput {...activeAccountForm.getInputProps('birthday')} required label="Date of Birth" placeholder="Enter Date of Birth" />
          </Group>
          <Group grow justify="end">
            <TextInput {...activeAccountForm.getInputProps('student_id')} readOnly label="Student ID" placeholder="Enter Student ID" />
            <TextInput {...activeAccountForm.getInputProps('address')} label="Address" placeholder="Enter Address" />
          </Group>
          <Group grow justify="end">
            <TextInput {...activeAccountForm.getInputProps('email')} readOnly label="Email" placeholder="Enter Email" />
            <PasswordInput {...activeAccountForm.getInputProps('password')} label="Password" placeholder="Enter Password" />
          </Group>

          <div className="text-right">
            <Button type="submit" loading={loadingActiveAccount}>Save Changes</Button>
          </div>
        </form>
      </CustomModal>
            ...
            <Table.Tbody>
              {appointments?.map((row: AppointmentProps) => {
                return (
                  <Table.Tr>
                    <Table.Td >
                      <Text title={row.qrcode} truncate>{row.qrcode}</Text>
                    </Table.Td>
                    <Table.Td miw={300}>
                      <List listStyleType="disc">
                    ...
                    <Table.Td miw={110}>{row.appointment_date ? formatDate(new Date(row.appointment_date)) : '-'}</Table.Td>
                    <Table.Td miw={120}>{row.appointment_time || '-'}</Table.Td>
                    <Table.Td c={row.status.toLowerCase() === 'completed' ? 'green' : (row.status.toLowerCase() === 'cancelled' ? 'red' : 'blue')}>{row.status.toUpperCase()}</Table.Td>
                    <Table.Td>
      ...
      <CustomModal title="Forgot Password" opened={forgotPasswordState} onClose={closeForgotPasswordState}>
        <form onSubmit={forgotPasswordForm.onSubmit(forgotPasswordEventHandler)}>
          <TextInput {...forgotPasswordForm.getInputProps("email")} required type="email" label="Email Address" placeholder="Enter email address" />
          <div className="text-right">
            <Button loading={loadingForgotPassword} type="submit" mt={10}>Submit</Button>
          </div>
        </form>
      </CustomModal>

      <Header title="Appointment" rightSection={account ? (
        <Menu withArrow>
          <Menu.Target>
            <div className="flex items-center gap-x-2 px-3 py-2 select-none active:opacity-80 cursor-pointer border border-slate-100 rounded-sm hover:bg-slate-200 hover:border-slate-400">
              <Text size="xs">{toProper(`${account.firstname} ${account.lastname}`)}</Text>
              <IconUser size={16} />
            </div>
          </Menu.Target>
          <Menu.Dropdown style={{
            ...
            <Menu.Item onClick={() => {
              ...
              openAccountState()
            }} fz="sm" leftSection={<IconUser size={16} />}>Account</Menu.Item>
            <Menu.Item fz="sm" leftSection={<IconCapProjecting size={16} />} onClick={openAppointmentState}>Appointment</Menu.Item>
            <Menu.Item fz="sm" leftSection={<IconLogout2 size={16} />} onClick={logoutEventHandler}>Log Out</Menu.Item>
          </Menu.Dropdown>
        </Menu>
      ) : (
        <div ref={ref}>
          <Popover withArrow opened={popOverState} withinPortal={false}>
            <Popover.Target>
              <Button onClick={() => setPopOverState(true)} variant="outline" rightSection={<IconUser size={15} />}>Sign In</Button>
            </Popover.Target>
            <Popover.Dropdown style={{
              ...
              <Divider my={10} />
              <form onSubmit={accountForm.onSubmit(accountEventHandler)} className="pb-1 space-y-2">
                <TextInput {...accountForm.getInputProps('email')} type="email" label="Email Address" required placeholder="Enter your email address" />
                <PasswordInput {...accountForm.getInputProps('password')} label="Password" required placeholder="Enter your password" />
                <div className="text-right">
                  <p onClick={() => {
                    openForgotPasswordState()
                    setPopOverState(false)
                  }} className="text-xs mb-2 hover:underline text-blue-500 cursor-pointer active:opacity-50 select-none">Forgot Password</p>
                </div>
                <Divider />
                <div className="py-2">
                  <Text size="xs">Don't have an account? <NavLink to="registration" className="hover:underline text-blue-500">Register</NavLink></Text>
                </div>
                <Button type="submit" fullWidth mt={6} loading={loadingLogin}>Sign In</Button>
              </form>
            </Popover.Dropdown>
          </Popover>
        </div>
      ...
      <div className="w-full h-[calc(100%-3.5rem)] p-5">
        <Stepper styles={{
        ...
        }} active={active} onStepClick={setActive} allowNextStepsSelect={false}>
          {/* STEP 1 : STUDENT INFORMATION */}
          <Stepper.Step label="First step" description="Account Verification">
            {steps.step1 ? (
              <div>
                <Text c="green" mt={10} ff="montserrat-bold" size="xl">Account Found!</Text>
                <Text>You can now proceed.</Text>
              </div>
            ) : (
              <div>
                <Text mt={10} ff="montserrat-bold" size="xl">Login Your Account First</Text>
                <Text>You need to check and verify whether your account already exists before proceeding.</Text>
              </div>
            )}
            <div className="flex gap-x-1 mt-5">
              <Button onClick={async () => {
                if (!account) {
                  displayError('Error', 'Account Not Found. Please Login First.')
                  return;
                }
                setSteps((curr) => ({ ...curr, step1: account }))
              }} rightSection={<IconArrowRight size={16} />}>Get My Account Information</Button>
              <Button onClick={() => setSteps((curr) => ({
                ...curr,
                step1: undefined
              }))} color="red" disabled={!steps.step1}>Reset</Button>
            </div>
            {steps.step1 && (
              <div className="py-3">
                <Table withRowBorders>
                  <Table.Tbody>
                    <Table.Tr>
                      ...
                      <Table.Td>{toProper(`${steps.step1.firstname} ${steps.step1.lastname}`)}</Table.Td>
                    </Table.Tr>
                    <Table.Tr>
                    ...
                    <Table.Tr>
                      ...
                      <Table.Td>{formatDate(new Date(steps.step1.birthday))}</Table.Td>
                    </Table.Tr>
                    <Table.Tr>
                      <Table.Th>Addrress</Table.Th>
                      <Table.Td>{steps.step1.address || 'No Address Provided'}</Table.Td>
                    </Table.Tr>
                    <Table.Tr>
                ...
                <div className="flex items-center justify-center">
                  <Group justify="center" mt="xl">
                    <Button disabled variant="default" onClick={prevStep}>Back</Button>
                    <Button onClick={nextStep} disabled={!steps.step1}>Next step</Button>
                  </Group>
                </div>
              </div>
            )}
            ...
            <div>
              <Select
                ...
                data={sections.map((sec) => ({
                  label: `${(sec.course)} ${sec.year_level[0]}${sec.section}`.toUpperCase(),
              ...
              />
              <CheckboxCard mt={20}>
                <CheckboxGroup value={steps.step2} onChange={(values) => {
                  setSteps((curr) => ({
                    ...curr,
                    step2: values
                  }))
              ...
              <Textarea value={noteValue} onChange={(e) => setNoteValue(e.target.value)} mt={10} rows={3} label="Note or Message (Optional)" placeholder="Enter your message or note" />
            </div>
            <Group justify="center" mt="xl">
              <Button variant="default" onClick={prevStep}>Back</Button>
              <Button disabled={!(steps.step2 && steps.step2.length > 0 && selectedSection)} onClick={nextStep}>Next step</Button>
            </Group>
          </Stepper.Step>
          ...
          <Stepper.Step label="Third step" description="Appointment Date">
            ...
            <Group justify="center" mt="xl">
              <Button variant="default" onClick={prevStep}>Back</Button>
              <Button disabled={!(selectedDate && selectedTime)} onClick={nextStep}>Proceed</Button>
            </Group>
          </Stepper.Step>
          {/* STEP 4 : DOWNLOAD / END */}
          <Stepper.Step label="Final step" description="Download">
            <div>
              <Text ff="montserrat-bold" size="xl">Confirm Your Appointment</Text>
              {steps.step1 && (
                <div className="py-3">
              ...
              <Divider mb={15} />
              <Text my={5} ff="montserrat-bold">Reason of Appointment</Text>
              <List listStyleType="disc">
            ...
            <Group justify="center" mt="xl">
              <Button variant="default" onClick={prevStep}>Back</Button>
              <Button loading={loadingAppointment} onClick={downloadEvent} rightSection={<IconDownload size={16} />}>Download & Save Appoinment</Button>
            </Group>
          </Stepper.Step>
        </Stepper>
      </div>
