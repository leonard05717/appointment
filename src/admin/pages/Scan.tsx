import { Button, ButtonGroup, Divider, List, Loader, LoadingOverlay, MantineSize, Select, Text, Textarea, TextInput, Tooltip } from "@mantine/core";
import PageContainer from "../../components/PageContainer";
import { IconQrcode } from "@tabler/icons-react";
import { useForm } from "@mantine/form";
import supabase from "../../supabase";
import { useEffect, useState } from "react";
import { AppointmentProps, AppointmentTimeProps, DisabledDateProps, SectionProps, UserProps } from "../../types";
import { displayError, formatDate, formatDateAndTime, toProper } from "../../helpers/methods";
...
function ScanField(props: ScanField) {
  return (
    <Text ff="montserrat-medium" size={props.size || 'sm'}>{props.label}: <span className="underline">{props.value ? props.upper ? props.value.toUpperCase() : (!props.disableProper ? toProper(props.value) : props.value) : 'No Data Provided'}</span></Text>
  )
}
...
function Scan() {
  ...
  async function submitQrCodeEventHandler(rs: QRCodeFormProps) {
    ...
    const { data, error } = await supabase
      ...
      .single()
    if (error) {
      displayError('Not Found', `The appointment with QR code: ${rs.qrcode.toUpperCase()} was not found.`);
      setData(undefined)
  ...
  async function submitMessageEventHandler() {
    ...
    if (status === 'return') {
      ...
      if (!selectedDate || !selectedTime) {
        displayError('No Selected Return Date and Time', 'Please select return date and time first.')
        return;
      }

      const date = new Date(selectedDate.getFullYear(), selectedDate.getMonth(), new Date(selectedDate).getDate() + 1);
    ...
    <PageContainer title="Scan QR Code" rightSection={<Button size="xs" onClick={resetEventHandler}>Reset</Button>}>
          ...
          {status === "return" && (
            <div className="space-y-2">
              <DatePickerInput
                ...
                label="Return Date" placeholder="Select Return Date"
                value={selectedDate}
                ...
                excludeDate={(date) => {
                  ...
                  return disabledDates.some((v) => new Date(v.date).toDateString() == dateString) || date.getDay() === 0
                }}
              />
              <Select
                {...DefaultSelectProps}
      ...
      <div className="fixed md:bottom-5 bottom-0 p-5 px-3 md:p-0 md:right-10 right-0 flex bg-white md:bg-inherit w-full md:w-fit z-10">
        <div className="flex items-center gap-x-1">
          <Text className="font-montserrat-medium text-sm" mr={5}>Mark as:</Text>
          <ButtonGroup>
            <Button onClick={() => markEventHandler('completed')} color="green" disabled={data?.status.toLowerCase() === 'completed' || !data}>Completed</Button>
            <Button onClick={() => markEventHandler('cancelled')} color="red" disabled={data?.status.toLowerCase() === 'cancelled' || !data}>Cancelled</Button>
            <Button onClick={() => markEventHandler('pending')} color="blue" disabled={data?.status.toLowerCase() === 'pending' || !data}>Pending</Button>
            {data && new Date(data.appointment_date).toDateString() === new Date().toDateString() && (
              <Button disabled={!data} onClick={() => markEventHandler('return')} color="orange">Return</Button>
            )}
          </ButtonGroup>
        </div>
      </div>
      <div className="grid md:grid-cols-2 grid-cols-1 grid-rows-1 w-full gap-x-5 pb-12 px-2">
        <div>
          <form onSubmit={qrcodeForm.onSubmit(submitQrCodeEventHandler)}>
            <TextInput id="qrcode-field" autoFocus data-autofocus disabled={loadingSearch} maxLength={6} {...qrcodeForm.getInputProps('qrcode')} minLength={6} leftSection={<IconQrcode size={17} />} placeholder="Scan or Enter QR Code here..." size="lg" rightSection={loadingSearch && <Loader size='sm' />} />
          </form>
          {data ? (
            <>
              <Text ff="montserrat-bold" mt={15}>Status</Text>
              <Text className="uppercase" size="xl" c={data.status.toLowerCase() === 'completed' ? 'green' : (data.status.toLowerCase() === 'cancelled' ? 'red' : 'blue')}>{data.status}</Text>
              <Divider my={10} />
              <Text ff="montserrat-bold" mt={15}>Appointment Details</Text>
              <Divider my={10} />
              <div className="space-y-2 border-b border-b-slate-300 pb-3">
                ...
                <Divider my={15} />
                <Text mt={10} ff="montserrat-medium">Note:</Text>
                <Text size="sm">{data.note || 'No Note Found'}</Text>
                <Divider my={15} />
                <Text mt={10} mb={4} ff="montserrat-medium">Appointment Date and Time:</Text>
                <ScanField label="Date" value={data.appointment_date ? formatDate(new Date(data.appointment_date)) : 'None'} />
                <ScanField upper label="Time" value={data.appointment_time || 'None'} />
              </div>
            </>
          ) : (
          ...
          {data ? (
            <div>
              <div className="w-full border border-slate-300 px-4 py-3 rounded-md shadow-md">
                ...
                <Divider my={10} />
                <div className="space-y-2 border-b border-b-slate-300 pb-3">
                  <ScanField label="Student ID" disableProper value={data.user.student_id.toUpperCase()} />
                ...
                <Text ff="montserrat-bold" mt={12}>Section</Text>
                <Divider my={10} />
                <div className="space-y-2 border-b border-b-slate-300 pb-3">
                  <ScanField label="Course" disableProper value={data.section.course.toUpperCase()} />
                  <ScanField label="Year Level" disableProper value={data.section.year_level} />
                  <ScanField label="Section" disableProper value={data.section.section} />
                  <ScanField label="Code" disableProper value={`${data.section.course} ${data.section.year_level[0]}${data.section.section}`} />
                </div>
                <Text size="sm" c="dimmed" mt={13}>{formatDateAndTime(new Date(data.updated_at)).toString()}</Text>
              </div>
              {data.message ? (
                <Tooltip label="Message" withArrow>
                  <div className="text-xs text-slate-500 mt-3 border-b border-slate-300 p-2 rounded-sm">{data.message}</div>
                </Tooltip>
              ) : <Text size="xs" mt={10} c="dimmed">No Message Found.</Text>}
